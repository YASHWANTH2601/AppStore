...
class AppStore extends Component {
  state = {acviveTabId: tabsList[0].tabId, searchInput: ''}
  updateActiveTabId = tabId => {
    this.setState({acviveTabId: tabId})
  }
  onChangeInput = event => {
  ...
  getActiveTabApps = searchApps => {
    const {acviveTabId} = this.state
    const filteredList = searchApps.filter(
      eachapp => eachapp.category === acviveTabId,
    )
    return filteredList
  }
  getSearchResult = () => {
    const {searchInput} = this.state
    const searchResutls = appsList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(searchInput.toLocaleLowerCase()),
    )
    return searchResutls
  }
  render() {
    const {acviveTabId, searchInput} = this.state
    const filteredApps = this.getSearchResult()
    const filteredApp = this.getActiveTabApps(filteredApps)
    return (
          ...
          <ul className="tab_list_container">
            {tabsList.map(eachItem => (
              <TabItem
                ...
                key={eachItem.tabId}
                item={eachItem}
                isActive={acviveTabId === eachItem.tabId}
              />
            ))}
          </ul>
          <ul className="app_list_container">
